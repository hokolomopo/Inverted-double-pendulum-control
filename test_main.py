...
if __name__ == "__main__":
    ...
    # plt.legend()
    # # plt.savefig("results/graphs/training_continuous.eps")
    # plt.show()
    ...
    for d in data:
        if(d.p.batch_size == 1 and d.p.lr == 0.01 and d.p.gamma == 0.99 and d.p.sigma == 1):
            plt.plot(d.it * d.p.batch_size, build_moving_average(d.rew, 0.01))
            print("{} mean of {:1f} max of {:.0f}".format(d.p.get_model_name(), np.mean(d.al), max(d.al)))
    plt.xlabel("Number of batches")
    plt.ylabel("Cumulative reward")
    plt.savefig("results/graphs/oscillations.eps")
    plt.show()
    ...
    #     }
    # for d in data:
    #     dic["gamma"].append(d.p.gamma)
    #     dic["lr"].append(d.p.lr)
    #     dic["sigma"].append(d.p.sigma)
    ...
    #         arr = []
    #         print("{} = {}, mean alive time = {}".format(item, v, d["mean_alive"].mean()))
